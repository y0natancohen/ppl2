...
import { isPrimOp, CExp, PrimOp, VarDecl, isString } from './L3-ast';
import { isNumber, isBoolean } from 'util';
import { getErrorMessages, isError } from './error';
...
export const compoundSExpToArray = (cs: CompoundSExp, res: string[]): string[] | { s1: string[], s2: string } =>
    isEmptySExp(cs.val2) ? res.concat([valueToString(cs.val1)]) :
